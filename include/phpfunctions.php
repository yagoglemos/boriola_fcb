...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="pedigree" && $field=="id")
	{
		
$ano = date("Y", strtotime($data['data']));
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="anilha" && $field=="clube")
	{
		return $_SESSION['clube'];
	}
				if($table=="federacao" && $field=="ativo")
	{
		return "SIM";
	}
				if($table=="pombo" && $field=="ativo")
	{
		return "SIM";
	}
				if($table=="socio" && $field=="datacricad")
	{
		return now();;
	}
				if($table=="socio" && $field=="ativo")
	{
		return "SIM";
	}
				if($table=="socio" && $field=="clube")
	{
		return $_SESSION['clube'];
	}
				if($table=="vanilhas" && $field=="quantidade")
	{
		return 0;
	}
				if($table=="vanilhas" && $field=="clube")
	{
		return $_GET['clube'];
	}
				if($table=="pedigree" && $field=="data")
	{
		return now();;
	}
	return "";
...
function runner_mail_smtp( $params )
{
	...
	try
	{
		...
		if(!isset($params['fromName'])) {
			...
			$mail->setFrom(GetGlobalData("strFromEmail", ""), GetGlobalData("strFromName", ""));
