...
##foreach @BUILDER.Tables as @t##
	##foreach @t.arrFieldObj as @f##
		##foreach @f.viewFormats as @vf filter @vf.strViewFormat==FORMAT_MAP && @vf.bUseMarkerIconExpr == 1##
	if($table=="##@t.strDataSourceTable s##" && $field=="##@f.strName s##")
	{
		##@vf.strMarkerIcon##;
		return $icon;
	}
...
{
	$icon = "";
##foreach @BUILDER.Tables as @t filter @t.nType==titDASHBOARD##
	##foreach @t.pDashboard.arrElements as @e filter @e.nType == DASHBOARD_MAP && @e.bUseMarkerIconExpr##
	if( $dashName == "##@t.strDataSourceTable s##" && $dashElementName == "##@e.strName g##" )
	{
		##@e.strMarkerIcon##;
		return $icon;
	}
...
function getDashMapCustomLocationIcon( $dashName, $dashElementName, $data ) 
{
	$icon = "";
##foreach @BUILDER.Tables as @t filter @t.nType==titDASHBOARD##
	##foreach @t.pDashboard.arrElements as @e filter @e.nType == DASHBOARD_MAP && @e.bUseLocationIconExpr##
	if( $dashName == "##@t.strDataSourceTable s##" && $dashElementName == "##@e.strName g##" )
	{
		##@e.strLocationIcon##;
		return $icon;
...
##foreach @BUILDER.Tables as @t##
	##foreach @t.arrFieldObj as @f##
		##foreach @f.viewFormats as @vf filter @vf.strViewFormat==FORMAT_CUSTOM##
			##if @f.bSeparateEditAndView##
	if($table=="##@t.strDataSourceTable s##" && $field=="##@f.strName s##" && $ptype=="##@vf.page##")
	{
		##@vf.strCustomExpr##;
		return $value;
	}
...
##foreach @BUILDER.Tables as @t##
	##foreach @t.arrFieldObj as @f##
		##foreach @f.viewFormats as @vf filter @vf.strViewFormat==FORMAT_FILE##
			##if @f.bSeparateEditAndView##
	if($table=="##@t.strDataSourceTable s##" && $field=="##@f.strName s##" && $ptype=="##@vf.page##")
	{
		##@vf.strFileCustomExpr##;
		return $value;
	}
			##else##
	if($table=="##@t.strDataSourceTable s##" && $field=="##@f.strName s##")
...
##foreach @BUILDER.Tables as @t##
##foreach @t.arrFieldObj as @f##
##foreach @f.editFormats as @ef filter (@ef.strEditFormat==EDIT_FORMAT_LOOKUP_WIZARD || @ef.strEditFormat==EDIT_FORMAT_RADIO) && @ef.pLookupObj.strWhere && iscodeexpression(@ef.pLookupObj.strWhere)##
	##if @f.bSeparateEditAndView##
	if($table=="##@t.strDataSourceTable s##" && $field=="##@f.strName s##" && $ptype=="##@ef.page##")
	{
		$strWhere = ##@ef.pLookupObj.strWhere##;
		return $strWhere;
	}
	##else##
	if($table=="##@t.strDataSourceTable s##" && $field=="##@f.strName s##")
...
##foreach @BUILDER.Tables as @t##
	##foreach @t.arrFieldObj as @f##
		##foreach @f.editFormats as @ef filter @ef.strDefaultValue.len##
			##if @f.bSeparateEditAndView##
	if($table=="##@t.strDataSourceTable s##" && $field=="##@f.strName s##" && $ptype=="##@ef.page##")
	{
		return ##@ef.strDefaultValue##;
	}
			##else##
	if($table=="##@t.strDataSourceTable s##" && $field=="##@f.strName s##")
	{
...
##foreach @BUILDER.Tables as @t##
	##foreach @t.arrFieldObj as @f##
		##foreach @f.editFormats as @ef filter @ef.strAutoUpdateValue.len##
			##if @f.bSeparateEditAndView##
	if($table=="##@t.strDataSourceTable s##" && $field=="##@f.strName s##" && $ptype=="##@ef.page##")
	{
		return ##@ef.strAutoUpdateValue##;
	}
			##else##
	if($table=="##@t.strDataSourceTable s##" && $field=="##@f.strName s##")
	{
...
{
##foreach @BUILDER.Tables as @t##
	##foreach @t.arrFieldObj as @f filter @f.bUploadCodeExpression##
	if($table=="##@t.strDataSourceTable s##" && $field=="##@f.strName s##")
	{
		##@f.codeUploadFolder##;
		return $folder;
...
{
	include_once(getabspath("classes/controls/".$className.".##@ext##"));
...
function importTableInfo( $varname ) {
	global $dal_info;
	include_once( getabspath( "include/dal/" . $varname . ".##@ext##" ) );
}
