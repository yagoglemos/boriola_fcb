...
class DropboxFileSystem extends OAuthFileSystem {
     ...
     * @return Array
     */
    protected function getUploadStrategy() {
        $strategy = array(
            "mode" => "add",
            "autorename" => true,
            "mute" => true,
     ...
     */
    public function saveShortData( $fileData, $userFilename ) {
        $strategy = $this->getUploadStrategy();
        $strategy[ "path" ] = DropboxFileSystem::normalizePath( $this->path ) . $userFilename;
    ...
    public function initUpload( $userFilename ) {
        ...
        $uniquePath = $this->tryGetUniqueObjectName( $userFilename, $normalizedPath );
        if( !$uniquePath ) {
            return false;
        }
        $strategy[ "path" ] = $normalizedPath . $uniquePath;
        $params = array(
