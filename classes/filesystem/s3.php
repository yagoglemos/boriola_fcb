...
class S3FileSystem extends RunnerFileSystem {
	...
	public function saveUploadedFile( $uploadFile, $userFilename ) {
		$reqTimestamp = time();
		$uniqueFilename = $this->tryGetUniqueObjectName( $userFilename, $this->path );
		if( !$uniqueFilename ) {
			$this->errorMessage = "Unable to create file in " . $this->path; 
			return null;
		}
		...
		$request->body = RunnerFileSystem::uploadedFileContent( $uploadFile );
