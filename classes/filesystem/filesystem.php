...
class RunnerFileSystem {
	...
	/**
	 * @return String unique resource key
	 */
	public function tryGetUniqueObjectName( $filename, $path ) {
		...
		do { 
			...
			$resourceKey = $path . $filename; 
			if( !$this->getFileInfo( $resourceKey ) )
				return $filename;
		} while( ++$atempt < 10 ); 

		$this->setLastError( "Unable to get unique file name for " . $filename . " at " . $path );
		return null;
	}
}
